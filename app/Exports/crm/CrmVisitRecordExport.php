...
class CrmVisitRecordExport implements FromCollection, WithTitle, WithEvents
{
    /**
    ...
    */
    ...
    public function collection(){
        ...
        foreach($this->data as $v){
            $DB_arrry[] = array(
                '訪談日期' => $v->visit_date,
                '報告日期' => $v->visit_report_date,
                '種類' => $v->visit_type,
                '姓名' => $v->c_name_company,
                '內容' => $v->visit_content,
                '建立者' => $v->creator_name,
                'AO' => $v->ao_staff_name,
                '客管' => $v->customer_analysis_name,
                '主管' => $v->supervisor_suggest_name,
            );
        }
